...
public:
    ...
    bool hit(const ray &r, interval ray_t, hit_record &rec) const override
    {
        ...
        ray rotated_r(local_origin, local_direction, r.time());

        // Check if corrected ray hits object

        if (!object->hit(rotated_r, ray_t, rec))
            return false;

        // Convert intersection and normal from local space to world space
